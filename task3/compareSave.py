    ...
    for i in validated_data_from_api():
        ...
        if list_db != list_api:
            ...
            for i in range(len(list_db)):
                print(list_db[i], list_api[i])
                print('____________________________________________')

    return list_of_changed_data


list_of_regno_be_changed = regno_changed()
list_of_rows_be_changed = []

list_of_new_data = validated_data_from_api()

for i in list_of_new_data:
    if i.reg_no in list_of_regno_be_changed:
        list_of_rows_be_changed.append(i)


# delet old row and add new row
def update_db():
    for regNo in list_of_regno_be_changed:
        try:
            row = Fp_data.objects.get(reg_no=regNo)
            row.delete()
        except:
            pass

    for item in list_of_rows_be_changed:
        Fp_data.objects.create(
            reg_no=item.reg_no,
            name=item.name,
            rank_of_12_month=item.rank_of_12_month,
